...
| `tools/render_check.jl`, `tools/run_render_check.sh` | headless export |
...
trajectory logging to `tools/solver_comparison.jl`, re-run the same frozen
...
the tree *and* its provenance, and `tools/render_check.jl` confirms in a
...
`tools/render_check.jl` proves it in a fresh process:
...
1. **Trajectory logging** (FJ9.4) — needed for world-track overlays and
   speed/heading time series. Safe form: extend `tools/solver_comparison.jl`,
