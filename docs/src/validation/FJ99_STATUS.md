...
`tools/clean_bootstrap.sh` runs with a throwaway `JULIA_DEPOT_PATH` and
...
| `tools/reproducibility_manifest.jl`, `run_manifest.sh` | isolation matrix + manifest |
| `tools/clean_bootstrap.sh` | clean-depot bootstrap |
...
sentence in `docs/src/validation/FJ8_STATUS.md` and stopped there, because that was where the
