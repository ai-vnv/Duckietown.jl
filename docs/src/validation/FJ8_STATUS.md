...
| `tools/native_gen_check.jl` | the script that fresh process runs |
...
`configs/planning/seeds.yaml` fixes two disjoint sets, and a test asserts
they do not overlap:
...
The protocol was frozen in `configs/planning/evaluation.yaml` **before** the
...
The experiment is `tools/run_comparison.sh` → `tools/solver_comparison.jl`, run in
...
One, in the FJ8.0 scaffolding itself: `tools/native_gen_check.jl` put its
